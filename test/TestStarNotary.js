...
it("lets 2 users exchange stars", async () => {
  ...
  let tokenId1 = 6;
  let tokenId2 = 7;
  let firstStar = "My first Star";
  let secondStar = "My second Star";
  let instance = await StarNotary.deployed();
  await instance.createStar(firstStar, tokenId1, { from: owner });
  await instance.createStar(secondStar, tokenId2, { from: user2 });
  ...
  assert.equal(await instance.ownerOf.call(tokenId2), owner);
});

it("lets a user transfer a star", async () => {
  let recipient = accounts[1];
  let tokenId = 8;
  let newStarName = "My new Star";
  let instance = await StarNotary.deployed();
  await instance.createStar(newStarName, tokenId, { from: owner });
  assert.equal(await instance.ownerOf.call(tokenId), owner);
  await instance.transferStar(recipient, tokenId);

  assert.equal(await instance.ownerOf.call(tokenId), recipient);
});
